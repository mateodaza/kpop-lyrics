...
export default async function ChatAdminPage() {
  ...
  const [queueIds, recentMessages, mutes] = await Promise.all([prisma.$queryRaw<Array<{ id: string }>>`
    SELECT m."id" FROM "ChatMessage" m
    WHERE m."status" = 'held' OR (
      m."status" = 'visible' AND EXISTS (
        SELECT 1 FROM "ChatReport" r
        WHERE r."messageId" = m."id"
          AND (m."reviewedAt" IS NULL OR r."createdAt" > m."reviewedAt")
      )
    )
    ORDER BY m."createdAt" ASC, m."id" ASC LIMIT 100
  ...
  const messages = queueIds.length ? await prisma.chatMessage.findMany({
    where: { id: { in: queueIds.map((item) => item.id) } },
  ...
  const items = messages.map((message) => ({
    ...
    reports: message.reports.filter((report) => !message.reviewedAt || report.createdAt > message.reviewedAt).map((report) => report.reason),
  }));
