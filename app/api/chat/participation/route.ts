...
export async function POST(request: NextRequest) {
  ...
  const session = await getChatSession();
  if (!session) return NextResponse.json({ error: "Sign in required." }, { status: 401 });
  if (Number(request.headers.get("content-length") ?? 0) > 256) return NextResponse.json({ error: "Invalid agreement." }, { status: 413 });
  ...
  } catch { return NextResponse.json({ error: "Could not save chat agreement." }, { status: 503 }); }
}
