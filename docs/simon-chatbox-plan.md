...
- `lib/auth.ts` currently returns a local `Session` with `user` and `userId`. The separate shared Aegyo Accounts worktree preserves that shape and adds a provider session ID. Chat posting uses a sensitive shared-session read; local password sessions stay read-only. Mapped legacy users may retain `emailVerified = false`, so that local flag is not a valid gate for shared accounts. No account schema change is needed here. **Launch gate:** merge the shared Aegyo Accounts cutover before opening chat posting in production.
...
5. Mount a compact collapsed panel on mobile (roughly 10–15vh) with recent message preview and new-message badge. Expansion opens an accessible room; desktop uses a bottom-right panel. Provide loading, empty, rate-limit, moderation, and sign-in states.
...
- Before production: configure `OPENAI_API_KEY` and `CHAT_CLEANUP_SECRET`, schedule daily cleanup, review/apply `scripts/chat/additive-schema.sql` to the correct PostgreSQL catalog, merge shared-auth cutover, and verify shared-account posting in staging, including a mapped legacy user.
