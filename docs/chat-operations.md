...
4. Set a high-entropy `CHAT_CLEANUP_SECRET` on the Aegyo service and the `aegyo-chat-cleanup` Railway Function. The function runs `scripts/chat/cleanup-cron.ts` daily and posts to `CHAT_CLEANUP_URL`; alert on failed runs. The cleanup removes ordinary visible/removed messages after 30 days, held/reported messages and moderation events after 90 days, posting-attempt metadata after one day, and expired mutes. Versioned age/rules acceptance remains for the lifetime of an account and cascades when the local account is deleted. Switch `CHAT_CLEANUP_URL` from the preview to the live Aegyo origin after the PR merges, before removing the preview service.
